...
class Convolutional_Neural_Network(object):
    def __init__(self):
        ...
        def get_conv_layer(input_variables, number_kernels_in, number_kernels_out, layer_number, filter_x, filter_y,
                           filter_type):
            weights = self.weight_variable([filter_y, filter_x, number_kernels_in, number_kernels_out],
                                           self._model_name + "w_conv_" + layer_number)
        ...
        def connect_conv_layers(input_variables):
            ...
            output = get_conv_layer(input_variables, kernel_list[0], kernel_list[1], '1', filter_x, filter_y,
                                    FILTER_TYPE)
            # print output.get_shape(), 'Features 1'

            for i in range(1, len(kernel_list) - 1):
                output = get_conv_layer(output, kernel_list[i], kernel_list[i + 1], str(i + 1), filter_x, filter_y,
                                        FILTER_TYPE)
                # print output.get_shape(), 'Features ', i+1

            # Flatten output
            output = tf.reshape(output, [-1,
                                         resize_y * (resize_x - (number_of_kernels * filter_x) + number_of_kernels) *
                                         kernel_list[-1]])
            return output

        def get_nn_layer(input_variables, connections_in, connections_out, layer_number):
            weights = self.weight_variable([connections_in, connections_out],
                                           self._model_name + 'w_fc_' + str(layer_number + 1))
